...
class MovieModel(Movie, Model):
    def __init__(self, title: str, year: str, imdb_id: str, poster: str):
        super().__init__(title=title, year=year, imdb_id=imdb_id, poster=poster)
    ...
    def __repr__(self):
        return f"MovieModel(title={self.title}, year={self.year}, imdb_id={self.imdb_id}, poster={self.poster})"
...
if __name__ == '__main__':
    movie = MovieModel(title='The Matrix', year="1999", imdb_id='tt0133093', poster='https://www.imdb.com/title/tt0133093/mediaviewer/rm1828758528')
    print(movie)
    print(movie.to_dict())
    print(movie.to_json())
